...
export const envSchema = z.object({
  ...
  KITCHEN_PORT: z.coerce.number().int().positive().default(4002),
  KITCHEN_DATABASE_URL: z.string().min(1),
  RABBITMQ_URL: z.string().min(1),
  KITCHEN_GRPC_URL: z.string().default("localhost:5002"),
  PORTION_RESERVATION_TTL_SECONDS: z.coerce.number().int().positive().default(600),
});
